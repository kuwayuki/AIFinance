...
PROMPT_USER = {
    ...
    """,
    "ANALYST_EVAL": """
    下記は{ticker}のアナリスト評価データと最新ニュース、保有情報です。
    それぞれ客観的に評価し、以下の1～8について将来の株価に関する見解を教えてください。
    （ニュースが極めてネガティブであれば、Analyst Price Targetsの数値は無視または割り引いて考慮してください）
    ...
    質問事項:
    1. 評価数 (10点満点)
    2. 短期的な株価上昇性 (約1～3か月を想定、10点満点)
    3. 長期的な株価上昇性 (1年以上先を想定、10点満点)
    4. 推奨購入価格 (現在の市場価格も参考に)
    5. スイングトレード(2週間以内)の売り価格
    - A. 目標価格
    - B. 到達確率(％)
    - C. 時期（yyyy/mm/dd）※今日から2週間後の目安
    6. 現実的な売り価格
    - A. 目標価格
    - B. 到達確率(％)
    - C. 時期（yyyy/mm/dd）
